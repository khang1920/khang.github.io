...
class Zefoy:
    ...
    def get_table(self, i = 1):
        ...
        for service in self.services:
            ...
            i += 1
        table.title = f"{Fore.WHITE} Các chức năng khác phải tích bằng VNĐ ((= Kkhang zZ: {len([x for x in self.services_status if self.services_status[x]])}{Fore.RESET}"
        print(table)
        ...
        while True:
            ...
            if 'Session expired. Please re-login' in self.video_info:
                ...
                while time.time()-2<_+zefoy:
                    t-=1
                    print('\033[36m[Kkhang zZ]=>Please what: \033[1;32m{0}\033[31m '.format(t)+'second\033[1;32m',end='\r')
                    time.sleep(1)
                continue
